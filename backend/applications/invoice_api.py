...
def parse_date(date_input):
    if isinstance(date_input, (int, float)):
        ...
        return datetime(date_input / 1000).date()
...
class InvoiceCore:
    ...
    def _generate_invoice_pdf(self, data, entity_type, start_date, end_date, invoice_number):
            class PDF(FPDF):
                def header(self):
...
class InvoiceListResource(Resource, InvoiceCore):
    ...
    def post(self): 
        ...
        existing = Invoice.query.filter(
            ...
            Invoice.status != 'cancelled',  # Exclude cancelled invoices
            or_(
...
class InvoiceStatusResource(Resource):
    @check_permission()
    def patch(self, invoice_id):
        ...
        # Apply stamp to PDF if status changed to paid/cancelled
        if new_status in ['paid', 'cancelled'] and old_status != new_status:
            ...
            # Overwrite the existing file with stamped version
...
class InvoiceDownloadResource(Resource):
    @check_permission()
